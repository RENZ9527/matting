...
class FluxLoRAConverter:
    ...
    def align_to_opensource_format(state_dict, alpha=None):
        ...
        for name, param in state_dict.items():
            # 0) 只处理 blocks/single_blocks 的 LoRA 权重
            if not (name.startswith("blocks.") or name.startswith("single_blocks.")):
                continue
            ...
            # 1) 长度保护：至少要有 prefix + block_id
            if len(names) < 4:
                # 可选：print一下方便定位脏key
                # print("[align_to_opensource_format] skip(short):", name)
                continue

            prefix = names[0]
            if prefix not in prefix_rename_dict:
                continue

            block_id = names[1]  # 现在不会越界了

            # 2) 兼容 .default. 这种多出来的层级
            # 目标：让最后两段变成 lora_A.weight / lora_B.weight
            # 常见情况：
            #   ... lora_A default weight   -> 去掉 default
            #   ... lora_A weight           -> 正常
            if len(names) >= 3 and names[-2] not in ("lora_A", "lora_B"):
                # 如果倒数第二段不是 lora_A/B，尝试去掉一个中间段（通常是 default）
                # 例如: [..., 'lora_A', 'default', 'weight']
                # names[-3] == lora_A/B 时 pop(-2) 去掉 default
                if len(names) >= 4 and names[-3] in ("lora_A", "lora_B") and names[-1] == "weight":
                    names.pop(-2)
                else:
                    continue

            # 3) 解析 middle/suffix
            middle = ".".join(names[2:-2])     # e.g. attn.a_to_qkv / ff_a.0 / norm1_a.linear ...
            suffix = ".".join(names[-2:])      # lora_A.weight / lora_B.weight

            if middle not in middle_rename_dict:
                continue
            if suffix not in suffix_rename_dict:
                continue

            rename = (
                prefix_rename_dict[prefix]
                + "_" + str(block_id)
                + "_" + middle_rename_dict[middle]
            ...
            if rename.endswith("lora_up.weight"):
                lora_alpha = alpha if alpha is not None else param.shape[-1]
                state_dict_[rename.replace("lora_up.weight", "alpha")] = torch.tensor(lora_alpha, device=param.device, dtype=param.dtype)
