...
from utils.cycle_loss import get_cycle_consistency_normal_loss,get_cycle_consistency_depth_loss,get_cycle_consistency_matting_loss,get_disperse_loss,ScaleAndShiftInvariantLoss,get_cycle_consistency_matting_loss_no_trimap
...
class FluxImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16,tokenizer_1_path=None, tokenizer_2_path=None):
        super().__init__(
        ...
        if tokenizer_1_path is None:
            tokenizer_1_path = "./FLUX.1-Kontext-dev/tokenizer"
        if tokenizer_2_path is None:
            tokenizer_2_path = "./FLUX.1-Kontext-dev/tokenizer_2"
            
        self.prompter = FluxPrompter(
            tokenizer_1_path=tokenizer_1_path, 
            tokenizer_2_path=tokenizer_2_path
        )
    ...
    def training_loss(self, **inputs):
        ...
        if inputs.get("extra_loss", None) is not None:
            ...
            #     print('using cycle_consistency_matting_estimation with trimap!!!!')
            #     cycle_consistency_loss = get_cycle_consistency_matting_loss(decoded,inputs["input_image"], inputs.get("trimap", None))
            # elif inputs.get("extra_loss", None) == "cycle_consistency_matting_estimation_no_trimap":
            #     print('using cycle_consistency_matting_estimation without trimap!!!!')
            #     cycle_consistency_loss = get_cycle_consistency_matting_loss_no_trimap(decoded,inputs["input_image"])
            # return flow_loss, cycle_consistency_loss
            # return cycle_consistency_loss.to(torch.bfloat16)
            print('using joint loss: flow + extra_loss!!!!')
            if inputs.get("extra_loss",None) == "cycle_consistency_matting_estimation":
                pixel_loss = get_cycle_consistency_matting_loss(decoded,inputs["input_image"], inputs.get("trimap", None))
            elif inputs.get("extra_loss", None) == "cycle_consistency_matting_estimation_no_trimap":
                pixel_loss = get_cycle_consistency_matting_loss_no_trimap(decoded,inputs["input_image"])
            
            # ---------------------------------------------------------
            # 单步专属：基于阶段的动态联合权重 (Warm-up + Ramp-up)
            # ---------------------------------------------------------
            if not hasattr(self, '_internal_train_step'):
                self._internal_train_step = 95000
            self._internal_train_step += 1
            
            # 定义关键步数节点
            WARMUP_STEPS = 5000.0   # 前 5000 步只用基础权重，让潜空间回归大结构
            RAMP_STEPS = 10000.0    # 用 10000 步的时间（即第 5000 到 15000 步）快速拉满权重
            
            start_lambda = 0.05  # 潜空间主导阶段的基础权重
            end_lambda = 1.5     # 像素级边缘细节主导阶段的最终权重
            
            if self._internal_train_step <= WARMUP_STEPS:
                # 第一阶段：Warm-up，保持较低权重
                final_lambda = start_lambda
            elif self._internal_train_step <= (WARMUP_STEPS + RAMP_STEPS):
                # 第二阶段：显著拉升阶段
                # 计算在这个爬坡阶段内的相对进度 (0.0 到 1.0)
                ramp_progress = float(self._internal_train_step - WARMUP_STEPS) / RAMP_STEPS
                # 使用线性增长（匀速拉升）
                final_lambda = start_lambda + (end_lambda - start_lambda) * ramp_progress
            else:
                # 第三阶段：第 15000 步之后直到结束，一直保持最大权重，死磕细节
                final_lambda = end_lambda
            
            if self._internal_train_step % 100 == 0:
                print(f"[Single-Step Tracker] Step {self._internal_train_step} | Pixel Lambda: {final_lambda:.4f}")
            
            # 总损失 = 潜空间单步回归损失 + 动态放大的像素边缘损失
            total_loss = flow_loss + final_lambda * pixel_loss.to(torch.bfloat16)
            
            return total_loss
        print('using flow loss!!!')
        return flow_loss
    ...
    def from_pretrained(
        ...
        if model_base_path is not None:
            tokenizer_1_path = os.path.join(model_base_path, "tokenizer")
            tokenizer_2_path = os.path.join(model_base_path, "tokenizer_2")
        else:
        ...
        # Initialize pipeline
        pipe = FluxImagePipeline(device=device, torch_dtype=torch_dtype,tokenizer_1_path=tokenizer_1_path,tokenizer_2_path=tokenizer_2_path)
