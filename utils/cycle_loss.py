...
MATTING_LAPLACIAN_WEIGHT = 0.2
MATTING_LAPLACIAN_LEVELS = 3
...
def _gaussian_blur_5x5(image: torch.Tensor) -> torch.Tensor:
    """
    Lightweight separable 5x5 Gaussian blur for pyramid construction.
    Expects image shape [B, 1, H, W].
    """
    ...
    return F.conv2d(image, kernel_2d, padding=2)


def _build_laplacian_pyramid(image: torch.Tensor, levels: int = MATTING_LAPLACIAN_LEVELS):
    """
    Builds a simple Laplacian pyramid. Returns a list from fine to coarse.
    """
    current = image
    pyramid = []
    for _ in range(levels - 1):
        blurred = _gaussian_blur_5x5(current)
        down = F.avg_pool2d(blurred, kernel_size=2, stride=2, ceil_mode=False)
        up = F.interpolate(down, size=current.shape[-2:], mode="bilinear", align_corners=False)
        up = _gaussian_blur_5x5(up)
...
def _multiscale_laplacian_loss_per_sample(
    pred_alpha: torch.Tensor,
    gt_alpha: torch.Tensor,
    mask: torch.Tensor,
    levels: int = MATTING_LAPLACIAN_LEVELS,
):
    """
    Computes masked multi-scale Laplacian pyramid loss and returns per-sample values.
    All tensors are expected in [B, 1, H, W].
    """
    pred_pyramid = _build_laplacian_pyramid(pred_alpha, levels=levels)
    gt_pyramid = _build_laplacian_pyramid(gt_alpha, levels=levels)

    losses = []
    current_mask = mask.float()

    for level_idx, (pred_level, gt_level) in enumerate(zip(pred_pyramid, gt_pyramid)):
        if level_idx > 0:
            current_mask = F.interpolate(current_mask, size=pred_level.shape[-2:], mode="nearest")

        diff = torch.abs(pred_level - gt_level)
        valid = current_mask.sum(dim=(1, 2, 3)).clamp_min(1.0)
        losses.append((diff * current_mask).sum(dim=(1, 2, 3)) / valid)
...
def get_cycle_consistency_matting_loss(pred_alpha, gt_alpha, trimap):
    ...
    # --- 2. Paper-style matting consistency: L1 on unknown + known regions ---
    abs_error = torch.abs(pred_alpha - gt_alpha)
    unknown_count = unknown_mask.sum(dim=(1, 2, 3)).clamp_min(1.0)
    known_count = known_mask.sum(dim=(1, 2, 3)).clamp_min(1.0)

    loss_l1_unknown_per_sample = (abs_error * unknown_mask).sum(dim=(1, 2, 3)) / unknown_count
    loss_l1_known_per_sample = (abs_error * known_mask).sum(dim=(1, 2, 3)) / known_count

    # --- 3. Keep the existing auxiliary objectives on the unknown region ---
    error_map_sad = abs_error
    loss_sad_per_sample = (error_map_sad * unknown_mask).sum(dim=(1, 2, 3)) / 1000.0

    # --- 4. MSE Loss (Mean Squared Error) ---
    error_map_mse = (pred_alpha - gt_alpha) ** 2
    loss_mse_per_sample = (error_map_mse * unknown_mask).sum(dim=(1, 2, 3)) / unknown_count
    ...
    # 计算梯度幅值
    pred_amp = torch.sqrt(pred_gx**2 + pred_gy**2+ 1e-8)
    gt_amp = torch.sqrt(gt_gx**2 + gt_gy**2+ 1e-8)

    # 计算梯度损失
    error_map_grad = (pred_amp - gt_amp) ** 2
    loss_grad_per_sample = (error_map_grad * unknown_mask).sum(dim=(1, 2, 3)) / 1000.0

    # --- 6. Multi-scale Laplacian Pyramid Loss ---
    loss_lap_per_sample = _multiscale_laplacian_loss_per_sample(
        pred_alpha, gt_alpha, unknown_mask, levels=MATTING_LAPLACIAN_LEVELS
    )

    # Paper-consistent core:
    #   E_U |y - y_hat| + E_K |y - y_hat|
    # Plus the retained auxiliary matting terms for stronger edge/detail quality.
    total_loss_per_sample = (
        loss_l1_unknown_per_sample
        + loss_l1_known_per_sample
        + loss_sad_per_sample
        + loss_mse_per_sample
        + loss_grad_per_sample
        + MATTING_LAPLACIAN_WEIGHT * loss_lap_per_sample
    )
