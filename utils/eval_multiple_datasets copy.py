...
DATASETS_MATTING: List[DatasetConfig] = [
    ...
    DatasetConfig(
        ...
        gt_path="/opt/liblibai-models/user-workspace2/users/rzc/datasets/AM-2k/",
    ...
    DatasetConfig(
        ...
        gt_path="/opt/liblibai-models/user-workspace2/users/rzc/datasets/OpenDataLab___P3M-10k/raw/P3M-10k/",
        output_dir="result/p3m_matting/",
        dataset_arg="p3m-np",
    ),
    DatasetConfig(
        ...
        gt_path="/nvmedata/workspace2/users/rzc/datasets/AIM-500",
...
def evaluate_dataset(pipe,
    ...
    # ========================== 过滤未处理样本 ==========================
    unprocessed_files = []
    unprocessed_trimaps = [] if trimaps is not None else None
    unprocessed_alphas = [] if alphas is not None else None

    for i, f in enumerate(files):
        rel_out = Path(f).relative_to(Path(ds_cfg.gt_path))
        save_to = (out_base / rel_out).with_suffix(".npy")

        if not save_to.exists():
            unprocessed_files.append(f)
            if trimaps is not None:
                unprocessed_trimaps.append(trimaps[i])
            if alphas is not None:
                unprocessed_alphas.append(alphas[i])

    files = unprocessed_files
    if trimaps is not None:
        trimaps = unprocessed_trimaps
    if alphas is not None:
        alphas = unprocessed_alphas

    print(f"[{ds_cfg.name}] Found {len(files)} unprocessed images remaining.")

    skip_generation = (len(files) == 0)
    if skip_generation:
        print(f"[{ds_cfg.name}] All images have been processed. Skip generation and continue to evaluation.")
    # ==================================================================
    ...
    if not skip_generation:
        ...
        if trimaps is not None:
            # Matting
            for batch_files, alpha_files in tqdm(
                zip(batches, alpha_batches),
                desc=f"generating {ds_cfg.name}",
                total=len(batches),
                disable=True
            ):
        ...
        print(f"Generation skipped for {ds_cfg.name}. Using existing predictions in: {ds_cfg.output_dir}")
...
def main():
    ...
    parser.add_argument("--task", type=str, default="matting", choices=["depth", "normal","matting"], help="评估任务类型，决定用哪个 DATASETS 列表")
